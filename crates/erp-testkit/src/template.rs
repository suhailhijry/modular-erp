...
async fn ensure_template(name: &str, schema: &Schema) -> anyhow::Result<()> {
    const MARKER: &str = "_erp_testkit_template_ready";
...
mod tests {
    ...
    fn age_is_read_from_the_name() {
        let now = 10_000;
        assert_eq!(age_from_name("erp_test_4000_123_7", now), Some(6_000));
        // Clock skew must not produce a huge age that sweeps a live database.
        assert_eq!(age_from_name("erp_test_99999_1_0", now), Some(0));
        assert_eq!(age_from_name("erp_test_notanumber_1_0", now), None);
