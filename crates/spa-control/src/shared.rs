...
const CHANNEL: &str = "spa:invalidate";
...
impl Shared {
    ...
    pub async fn forget_sessions_of(&self, identity: IdentityId) {
        ...
        for digest in &digests {
            pipe.del(format!("spa:session:{digest}")).ignore();
        }
...
fn session_key(digest: &[u8]) -> String {
    format!("spa:session:{}", hex::encode(digest))
}

fn owner_key(identity: IdentityId) -> String {
    format!("spa:sessions-of:{identity}")
}
...
mod tests {
    ...
    fn keys_are_namespaced_and_distinct() {
        ...
        assert!(session_key(b"x").starts_with("spa:"));
        assert!(owner_key(a).starts_with("spa:"));
    }
}
