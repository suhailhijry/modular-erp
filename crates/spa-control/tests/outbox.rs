...
//!
//! `spa_eventlog`'s `Dispatcher` and `enqueue` are compile-time-checked against
...
async fn fresh(schema: &'static Schema) -> TestDb {
    spa_testkit::Template::get(schema)
        .await
...
async fn the_two_outboxes_are_the_same_table() {
    ...
    assert_eq!(
        ...
        "the two `outbox` tables have drifted. `spa_eventlog`'s queries are \
...
async fn the_control_plane_can_promise_an_effect_without_an_event_log() {
    ...
    let effect = spa_eventlog::Effect::with_key(
        spa_types::EffectKind::new("email.send").expect("a kind"),
        "invitation:abc",
        serde_json::json!({ "to": "sara@example.test" }),
    );

    let written = spa_eventlog::enqueue(&mut conn, None, std::slice::from_ref(&effect))
        .await
    ...
    let again = spa_eventlog::enqueue(&mut conn, None, &[effect])
        .await
        .expect("enqueues");
    ...
    let keyless = spa_eventlog::Effect::new(
        spa_types::EffectKind::new("email.send").expect("a kind"),
        serde_json::json!({}),
    );
    assert!(
        spa_eventlog::enqueue(&mut conn, None, &[keyless])
            .await
            .is_err(),
