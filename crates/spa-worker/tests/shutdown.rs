...
use spa_control::{Actor, ClusterRegistry, ControlPlane, PoolConfig, TenantPools, WorkSchedule};
use spa_eventlog::{
...
use spa_worker::{Activity, Job, ProjectionJob, Worker, WorkerConfig};
...
async fn a_failing_job_stalls_one_tenant_without_taking_the_worker_down() {
    ...
    impl Job for AlwaysFails {
        ...
        async fn tick(
            &self,
            _db: &spa_control::TenantDb,
        ) -> Result<Activity, spa_worker::BoxError> {
