...
use spa_testkit::{Schema, Template, TestDb, kill_connection};
use spa_types::{AggregateId, DomainName, EffectKind, EventName, SchemaVersion, Sequence};
...
async fn a_crash_during_an_append_burns_no_position() {
    ...
    assert_eq!(
        committed.at.map(spa_types::LogPosition::get),
        Some(1),
...
async fn a_delivery_whose_record_is_lost_happens_again_with_the_same_key() {
    ...
    assert_eq!(settlement, spa_eventlog::Settlement::Delivered);
    ...
    // settle did not.
    let (attempts, delivered): (i32, Option<spa_types::Timestamp>) =
...
async fn a_crash_during_a_claim_leaves_the_effect_owed() {
    ...
    let (attempts, leased): (i32, Option<spa_types::Timestamp>) =
