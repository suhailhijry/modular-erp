...
impl TwoNodes {
    async fn new() -> Self {
        ...
        .await
        .expect("registers");
        erp_control::shared::apply_invalidations_in_background(&a);
        erp_control::shared::apply_invalidations_in_background(&b);
...
async fn an_unreadable_invalidation_is_not_guessed_at() {
    let known = serde_json::to_string(&Invalidate::Identity(erp_types::IdentityId::new()))
        .expect("serializes");
