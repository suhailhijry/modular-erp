...
//!
//! ```sh
//! cargo test -p erp-control --test soak -- --ignored --nocapture
...
use erp_control::{Actor, ClusterRegistry, ControlPlane, Lane, PoolConfig, Scope, TenantPools};
...
async fn connections_track_operations_not_requests() {
    ...
    let sampler = {
        ...
        tokio::spawn(async move {
            let admin = sqlx::PgPool::connect(&erp_testkit::database_url())
                .await
            ...
            while !stop.load(Ordering::Relaxed) {
                let sample: Option<(i64, i64)> = sqlx::query_as(
                    "SELECT count(*),
                      ...
                      WHERE datname LIKE 'erp_tenant_%'",
                )
