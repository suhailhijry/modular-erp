...
impl erp_i18n::Localize for MemberError {
...
fn parse_module_role(pair: &str) -> Result<(erp_types::ModuleId, Role), AccessError> {
    let (module, role) = pair
        .split_once('=')
        .ok_or_else(|| AccessError::Corrupt(format!("module role {pair:?}")))?;
    Ok((
        erp_types::ModuleId::new(module.to_owned())
