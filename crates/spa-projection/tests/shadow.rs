...
use spa_eventlog::{DomainEvent, Envelope, Metadata, NewEvent, Upcasters, append, read_since};
use spa_projection::{
    Projection, ProjectionCtx, ProjectionError, ProjectionGroup, ensure_group_schema, run_to_head,
};
use spa_testkit::{Schema, Template, TestDb};
...
async fn seed(db: &TestDb, n: i64) {
    let mut conn = db.pool().acquire().await.expect("connection");
    let already = read_since(&mut conn, spa_types::LogPosition::ZERO, 10_000)
        .await
        .expect("reads")
        .len();
...
async fn a_rebuild_swaps_in_new_tables_without_ever_showing_none() {
    ...
    let reached = spa_projection::rebuild_swap::<Ledger>(db.pool(), V2, &totals, &upcasters(), 100)
        .await
    ...
    let mut conn = db.pool().acquire().await.expect("connection");
    let checkpoint = spa_projection::checkpoint::<Ledger>(&mut conn)
        .await
        .expect("reads");
    drop(conn);
...
async fn a_rebuild_catches_up_what_arrived_while_it_ran() {
    ...
    let reached = spa_projection::rebuild_swap::<Ledger>(db.pool(), V1, &totals, &upcasters(), 100)
        .await
        .expect("rebuilds");
...
async fn a_failed_rebuild_leaves_the_live_tables_alone() {
    ...
    let failed =
        spa_projection::rebuild_swap::<Ledger>(db.pool(), WRONG_SHAPE, &totals, &upcasters(), 100)
            .await;
...
async fn a_rebuild_refuses_schema_qualified_install_sql() {
    ...
    let refused =
        spa_projection::rebuild_swap::<Ledger>(db.pool(), QUALIFIED, &totals, &upcasters(), 100)
            .await;
