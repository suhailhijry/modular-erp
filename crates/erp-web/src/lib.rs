...
//! cannot name is one it cannot use, and a module that reached up into `erp-api`
//! to get one would close a dependency cycle, because `erp-api` names every
//! module.
//!
...
//! messages. `erp-api` is what is left — the core's own routes, the module
...
//! one — see any module's `http::problem_for`. `erp_api::CATALOG` is the union
...
pub use extract::{
    Allowed, Anonymous, Authenticated, BRANCH_HEADER, Capability, FORWARDED_FOR, IdempotencyKey,
    IfMatch, Language, ManageAccounts, ManageTenant, PostEntries, Public, Read, Tenant,
    tenant_of_host,
};
pub use problem::Problem;
pub use state::AppState;
pub use wire::{
    After, Amount, Json, Paged, Query, Versioned, bad_request, config_problem, creating, importing,
    metadata, parse_id, publicly, require_module,
};
...
mod tests {
    ...
    fn every_failure_this_crate_can_answer_with_is_translated() {
        erp_i18n::testing::assert_complete(&super::CATALOG);
    }
}
