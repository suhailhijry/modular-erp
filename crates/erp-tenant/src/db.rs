...
impl TenantDb {
    ...
    /// `erp-api` is the type-level form; this is what it calls.
    #[must_use]
    ...
    ///
    /// The retry loop lives here rather than in `erp-eventlog` because each
    ...
    ///
    /// See `erp_eventlog::try_execute` for what one attempt does.
        ...
        F: Fn(&erp_eventlog::Loaded<A>) -> Result<erp_eventlog::Decision<A::Event>, E>,
    {
        for attempt in 1..=erp_eventlog::MAX_ATTEMPTS {
            ...
            match erp_eventlog::try_execute::<A, _, E>(&mut tx, id, upcasters, metadata, &decide)
                .await
            {
                Ok(committed) => {
                    tx.commit()
...
/// while everything inside [`erp_eventlog::ExecuteError`] is about the command
...
pub enum CommandError<E> {
    ...
    Pool(#[from] PoolError),
    #[error(transparent)]
    Execute(#[from] erp_eventlog::ExecuteError<E>),
}
