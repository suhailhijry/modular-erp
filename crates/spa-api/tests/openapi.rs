...
/// The conventions in [`spa_api::openapi`]'s `Conventions` reached everything.
///
...
fn every_code_the_document_cites_exists() {
    let known: BTreeSet<&str> = spa_i18n::Catalog::codes(&spa_api::CATALOG)
        .iter()
        .map(spa_i18n::MessageCode::as_str)
        .collect();
