...
//!
//! Because the document might not be written. A create is idempotent on the key
//! the client sent in `Idempotency-Key`, so re-sending one is a no-op — and a
...
impl erp_i18n::Localize for NumberingError {
